...
class MapViewController: UIViewController {
    ...
    //Outlets to UIVIew
    @IBOutlet weak var nonprofitButton: UIButton!
    @IBOutlet weak var ClothingButton: UIButton!
    @IBOutlet weak var showersButton: UIButton!
    ...
    let simpleDataArray = ["Showers", "Clothing", "Non-Profit"]
    var currentMapState = ["Showers", "Clothing", "Non-Profit"]
    ...
    //Declarations for Side Menu
     ...
     var removeShowers = false
    var removeClothing = false
    var removeNonprofit = false
    ...
    let headerTitles = ["Showers", "Clothing", "Non-Profit"]
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 15.0, *) {
            serviceTable.sectionHeaderTopPadding = 0
        } else {
            // Fallback on earlier versions
        }
        serviceTable.contentInset.bottom = 100
    ...
    func updateMap(){
        ...
        if simpleSelectedArray.contains("Non-Profit") && !currentMapState.contains("Non-Profit"){
            mapView.addAnnotations(servicesList[2])
            currentMapState.append("Non-Profit")
        }
        else if !simpleSelectedArray.contains("Non-Profit") && currentMapState.contains("Non-Profit"){
            print("Removing annotations")
            mapView.removeAnnotations(servicesList[2])
            currentMapState.removeAll(where: { $0 == "Non-Profit" })
    ...
    @IBAction func buttonCollectionPress(_ sender: UIButton) {
        ...
        if sender.backgroundColor == UIColor(named: "LogoBlue1"){
            print("Light")
            sender.backgroundColor = UIColor(named: "DarkBlue")
            sender.setImage(UIImage(systemName: "multiply"), for: .normal)
            sender.tintColor = .lightGray
            sender.setTitleColor(.lightGray, for: .normal)
            updateTable(value: sender.titleLabel!.text!)
        }
        else {
            print("Dark")
            sender.backgroundColor = UIColor(named: "LogoBlue1")
    ...
    func updateTable(value : String){
        switch value{
        case "Showers":
            removeShowers = !removeShowers
            break
        case "Clothing":
            removeClothing = !removeClothing
            break
        case "Non-Profit":
            removeNonprofit = !removeNonprofit
            break
        default:
            break
        }
